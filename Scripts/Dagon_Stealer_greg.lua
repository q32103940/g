...
client.screenSize.x/1600
local F14         = drawMgr:CreateFont("F14","Tahoma",14*monitor,550*monitor) 
local debugText1  = 

drawMgr:CreateText(10*monitor,475*monitor,-1,"debug1",F14) debugText1.visible = true
local debugText2  = drawMgr:CreateText

(10*monitor,460*monitor,-1,"debug2",F14) debugText2.visible = true
local debugText3  = drawMgr:CreateText(10*monitor,445*monitor,-

1,"debug3",F14) debugText3.visible = true 
--Главная функция
function Tick(tick)
...
client.console or tick < sleep then return end
...
purify.cd==0 and GetDistance2D(v,me) < purify.castRange  then
...
--dagon only
if dagon and dagon.cd==0 and GetDistance2D(v,me) < dagon.castRange and v.health < v:DamageTaken(dmgD, DAMAGE_MAGC, me) 
...
relativeangle<0.05 and v.health < v:DamageTaken(dmgD+fortunedmg, DAMAGE_MAGC, me) then
me:SafeCastAbility(dagon,v, false)

--fortunes + 

purify + dagon
elseif dagon and dagon.cd==0 and GetDistance2D(v,me) < dagon.castRange and GetDistance2D(v,me) < purify.castRange and 

relativeangle<0.05 and v.health < v:DamageTaken(dmgD+fortunedmg+purifydmg, DAMAGE_MAGC,me) then
me:SafeCastAbility(dagon,v, false)
...
ethereal.cd==0 and purify and purify.cd==0 and v.health < v:DamageTaken

(((90*purify.level+75+2*attribute)*1.4), DAMAGE_MAGC, me)  then
...
dagon.cd==0 and v.health < v:DamageTaken(((dmgD
...
ethereal and ethereal.cd==0 and 

dagon.cd==0 and purify and purify.cd==0 and v.health < v:DamageTaken(((dmgD+75+2*attribute

+90*purify.level)*1.4), DAMAGE_MAGC, me) then
